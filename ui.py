...
import os
import math
os.chdir(r"C:\Users\Janit\TTC_NLG")
...
#  1. Bus → Station (NLG)
    ...
    if st.button("Search Bus", use_container_width=True):
        ...
        response = requests.get(f"http://127.0.0.1:8000/bus/{bus_search}")
...
if option == "Station":
    ...
    if st.button("Search", use_container_width=True):
        ...
        response = requests.get(f"http://127.0.0.1:8000/station/{station}")
...
response = requests.get("http://127.0.0.1:8000/bus_numbers")
if response.status_code == 200:
...
if selected_bus and st.button("Show Stops Table"):
    response_stops = requests.get(f"http://127.0.0.1:8000/bus/{selected_bus}/stops")
